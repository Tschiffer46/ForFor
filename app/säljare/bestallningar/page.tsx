            ...
            <Card key={order.id}>
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle className="text-lg">{order.kund.namn}</CardTitle>
                    <p className="text-sm text-gray-600 mt-1">
                      {order.kund.adress.gatuadress}
                    </p>
                    {order.kund.telefon && (
                      <p className="text-sm text-gray-600">{order.kund.telefon}</p>
                    )}
                  </div>
                  <Badge
              ...
              <CardContent className="space-y-3">
                  ...
                  {order.orderItems.map((item) => (
                    ...
                    >
                      ...
                      <span className="font-medium">
                        {formatCurrency(item.antal * item.styckpris)}
                      </span>
                    </div>
                  ))}
                  {order.orderItems.some((item) => item.rabattTillampad) && (
                ...
                <div className="pt-2 border-t text-xs text-gray-500 space-y-1">
                  <p>Säljrunda: {order.saljrunda.namn}</p>
